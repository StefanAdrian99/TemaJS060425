...
for (let i = 0; i < products.length; i++) {
    ...
    let description = products[i].description.toLowerCase();

    if (title.includes("slim") || description.includes("slim")) {
        slimProducts.push(products[i].id);
    }
}

console.log(slimProducts);

// EXERCITIUL 2

for(let i = 0; i < products.length; i++){
    if(products[i].category === "men's clothing" || products[i].category === "women's clothing"){
...
console.log(`The most expensive electronic is ${mostExpensiveElectronic.price}`);
...
console.log(`Cheapest womens clothing is ${cheapestWomenClothing.price}`);
...
console.log(`Highest electronic rating is ${highestElectronicRating.rating.rate}`);
...
console.log(`The most popular product is ${highestRating.title} and it rated of ${highestRating.rating.rate} out of 5 by ${highestRating.rating.count} reviewers`);